	...
	getTableLength = function(_table)
		local count = 0
		if not (_table == nil) then
	...
	switchTo = function(application)
		...
		if not OSFirstRunMode then
			OSUpdate()
			local appMenuItems = table.shallow_copy(application.mainMenu)

			if getTableLength(appMenuItems) < 1 then
				appMenuItems = {
				...
				if application.crashed then --yup, it crashed
					appMenuItems = {
						OSMenuItem:new("Quit "..application.title, function () application:quit() end, "Q")
					}
				end
			...
			elseif getTableLength(application.mainMenu) == getTableLength(appMenuItems) then
					table.insert(appMenuItems, OSMenuItem:new("Quit "..application.title, function () application:quit() end, "Q")) 				
			end
