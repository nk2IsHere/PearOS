--OSTextView--
	...
	new = function(self, _x, _y, _width, _height, _text, _submit)
	...
	Draw = function(self, darkMode)
		...
		for _,line in ipairs(self.lines) do
			...
			OSDrawing.DrawCharacters(self.x, self.y+_-1, line, textColour, backgroundColour)
			if startChar < self.Selection[1] then
				local selStr = string.sub(line,self.Selection[1],self.Selection[2])
				OSDrawing.DrawCharacters(self.x+self.Selection[1]-1, self.y+_-1, selStr, selectedTextColour, selectedBackgroundColour)
			else
				OSDrawing.DrawCharacters(self.x, self.y+_-1, line, textColour, backgroundColour)
			end			
			end
		...
		if self.isSelected then
			if self.Blink then
				...
				OSDrawing.DrawCharacters(cX, cY, self.BlinkCharacter, textColour, backgroundColour)
			end
